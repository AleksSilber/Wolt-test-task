...
function calculateHaversineDistance(lat1: number, lon1: number, lat2: number, lon2: number): number {
    const EARTH_RADIUS = 6371000; //converted from km to m
...
function updateUI(cart: number, distance: number, basePrice: number, minOrderPrice: number, distanceRanges: any[]) {
...
export default function SubmitButton() {
    async function calculateResult() {
        const venueSlug = document.getElementById("venueSlug")?.getAttribute("value");
        const cartValue = document.getElementById("cartValue")?.getAttribute("value");
        const userLat = document.getElementById("userLatitude")?.getAttribute("value");
        const userLon = document.getElementById("userLongitude")?.getAttribute("value");
        ...
        updateUI(Number(cartValue), distance, basePrice, minOrderPrice, distanceRanges);
    }

    return (
        <div>
            <button className="btn btn-primary m-3" onClick={calculateResult} type="submit">
                Calculate
            </button>
        </div>
    );
